	...
	for key in player_pools.keys():
		...
		while empty_matches.size() > 0 and player_pool.size() > 0:
			...
			for i in range(fill_range):
				...
				var fitting_player = HelperFunctions.swap_pop(player_pool, fitting_index)
				if fitting_player == null:
					push_error("Player assignment failed: No fitting player in bounds of current player pool: " + player_pool)
					return []
				empty_match.players.append(fitting_player)
			var filled_match = HelperFunctions.swap_pop(empty_matches, 0)
			filled_matches.append(filled_match)
		if filled_matches.size() == 0:
			push_error("Player assignment failed: No matches were filled")
			return []
		# match balancing
		if match_size > 2:
...
func _create_matches(number_of_players: int) -> Array[Match]:
	...
	for i in range(needed_matches):
		...
		# This ensures that stations get assigned corectly even if station count < needed matches
...
func _find_suitable_player_index(current_match: Match, player_pool: Array[Player]) -> int:
	var best_idx := -1
	var best_score := -1
...
func save() -> String:
	...
	if error == OK:
		print("Successfully saved swiss round %s to %s" % [str(swiss_round), self.resource_path])
		return "OK"
	else:
		var error_message = "Failed to save swiss round %s to %s. Error: %s" % [str(swiss_round), self.resource_path, error]
